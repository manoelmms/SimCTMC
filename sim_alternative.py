...
def simulation(arrival_rate, simulation_time, number_of_queues):
    ...
    while True:
        ...
        # Create a job with fixed service time (1 second)
        job_time = 1
...
if __name__ == "__main__":
    ...
    for arrival_rate in arrival_rate_list:
        result.append(simulation(arrival_rate, simulation_time, number_of_queues))

    plt.plot(arrival_rate_list, [r[0] for r in result], label='Mean Time in System', color = 'green')
    plt.title('Mean Time in System')
    plt.xlabel('Arrival Rate')
    plt.ylabel('Time')
    plt.show()

    # plt.plot(arrival_rate_list, [r[1] for r in result], label='Mean Waiting Time')
